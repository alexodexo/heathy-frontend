        ...
        <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
          ...
          <div className="space-y-3 mb-4 pb-4 border-b border-gray-300">
            ...
            <div className="flex items-end gap-4">
              <div className="flex-1">
                ...
                <input
                  type="number"
                  value={localSettings.heizung_switchon ?? '--'}
                  onChange={(e) => {
                    const value = parseFloat(e.target.value)
                    setLocalSettings(prev => ({ ...prev, heizung_switchon: value }))
                  }}
                  onBlur={(e) => {
                    const value = parseFloat(e.target.value)
                    if (value !== einstellungen?.heizung_switchon?.value) {
                      updateSetting('heizung_switchon', value)
                    }
                  }}
              ...
              <div className="flex-1">
                ...
                <input
                  type="number"
                  value={localSettings.heizung_switchoff ?? '--'}
                  onChange={(e) => {
                    const value = parseFloat(e.target.value)
                    setLocalSettings(prev => ({ ...prev, heizung_switchoff: value }))
                  }}
                  onBlur={(e) => {
                    const value = parseFloat(e.target.value)
                    if (value !== einstellungen?.heizung_switchoff?.value) {
                      updateSetting('heizung_switchoff', value)
                    }
                  }}
                  className="input text-gray-900 w-full"
                  min="20"
          ...
          <div className="space-y-3 mb-4 pb-4 border-b border-gray-300">
            ...
            <div className="flex items-center gap-3 mb-3">
              <input
                type="checkbox"
                id="heizung_coldstart_enabled"
                checked={!!localSettings.heizung_coldstart_enabled}
                onChange={(e) => {
                  const value = e.target.checked ? 1 : 0
                  setLocalSettings(prev => ({ ...prev, heizung_coldstart_enabled: value }))
                  updateSetting('heizung_coldstart_enabled', value, 'Heizung: Kaltstart-Boost aktiviert (0=false, 1=true)')
                }}
                className="w-4 h-4 text-primary-600 bg-gray-100 border-gray-300 rounded focus:ring-primary-500"
                disabled={isSaving || einstellungenLoading}
              />
              <label htmlFor="heizung_coldstart_enabled" className="text-sm font-medium text-gray-700 cursor-pointer">
                Booster aktivieren
              </label>
            </div>

            {/* Kaltstart-Boost Einstellungen */}
            {localSettings.heizung_coldstart_enabled && (
              <div className="pl-7 space-y-3">
                <div className="flex items-center gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <input
                        type="number"
                        value={localSettings.heizung_coldstart_target_temp ?? 45}
                        onChange={(e) => {
                          const value = parseFloat(e.target.value)
                          setLocalSettings(prev => ({ ...prev, heizung_coldstart_target_temp: value }))
                        }}
                        onBlur={(e) => {
                          const value = parseFloat(e.target.value)
                          if (value !== einstellungen?.heizung_coldstart_target_temp?.value) {
                            updateSetting('heizung_coldstart_target_temp', value)
                          }
                        }}
                ...
                <div className="mt-2 p-2 bg-green-50 rounded border border-green-200">
                  <p className="text-xs text-green-800">
                    <strong>⚡ Funktion:</strong> Die Heizung startet mit allen drei Heizstäben (4,5kW gesamt) und heizt auf, bis die Vorlauftemperatur {localSettings.heizung_coldstart_target_temp ?? 45}°C erreicht hat. Danach schaltet das System automatisch auf die normale Regelung um.
                  </p>
                </div>
              </div>
            )}
          ...
          <div className="space-y-3 mb-4 pb-4 border-b border-gray-300">
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <input
                  type="number"
                  value={localSettings.heizung_l2_boost_time ?? '--'}
                  onChange={(e) => {
                    const value = parseInt(e.target.value)
                    setLocalSettings(prev => ({ ...prev, heizung_l2_boost_time: value }))
                  }}
                  onBlur={(e) => {
                    const value = parseInt(e.target.value)
                    if (value !== einstellungen?.heizung_l2_boost_time?.value) {
                      updateSetting('heizung_l2_boost_time', value)
                    }
                  }}
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <input
                  type="number"
                  value={localSettings.heizung_l3_boost_time ?? '--'}
                  onChange={(e) => {
                    const value = parseInt(e.target.value)
                    setLocalSettings(prev => ({ ...prev, heizung_l3_boost_time: value }))
                  }}
                  onBlur={(e) => {
                    const value = parseInt(e.target.value)
                    if (value !== einstellungen?.heizung_l3_boost_time?.value) {
                      updateSetting('heizung_l3_boost_time', value)
                    }
                  }}
                  className="input text-gray-900 w-24"
                  min="0"
              ...
              <div className="flex items-center gap-4">
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      type="number"
                      value={localSettings.heizung_downshift_l3_offset ?? 3.0}
                      onChange={(e) => {
                        const value = parseFloat(e.target.value)
                        setLocalSettings(prev => ({ ...prev, heizung_downshift_l3_offset: value }))
                      }}
                      onBlur={(e) => {
                        const value = parseFloat(e.target.value)
                        if (value !== einstellungen?.heizung_downshift_l3_offset?.value) {
                          updateSetting('heizung_downshift_l3_offset', value)
                        }
                      }}
                      className="input text-gray-900 w-28"
                      min="0"
              ...
              <div className="flex items-center gap-4">
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      type="number"
                      value={localSettings.heizung_downshift_l2_offset ?? 1.5}
                      onChange={(e) => {
                        const value = parseFloat(e.target.value)
                        setLocalSettings(prev => ({ ...prev, heizung_downshift_l2_offset: value }))
                      }}
                      onBlur={(e) => {
                        const value = parseFloat(e.target.value)
                        if (value !== einstellungen?.heizung_downshift_l2_offset?.value) {
                          updateSetting('heizung_downshift_l2_offset', value)
                        }
                      }}
                      className="input text-gray-900 w-28"
                      min="0"
              ...
              <div className="flex items-center gap-4">
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      type="number"
                      value={localSettings.heizung_switchoff ?? 45}
